...
//    List<User> findUsersWithPositionAndLessThan3Jobs(@Param("type") int type);
    ...
    @Query("SELECT u FROM User u " +
            ...
            " WHERE j.user = u AND j.job_log = false AND " +
            "       (j.status.status_id != 6 AND j.status.status_id != 9 AND j.status.status_id != 12)) < 3")
    List<User> findUsersWithPositionAndLessThan3Jobs(@Param("type") int type);
    ...
    @Query("SELECT u FROM Jobs u WHERE u.code = :code AND u.status.status_id = 16")
    List<Jobs> getJobByJobCodeSick(String code);
    ...
    @Query("SELECT u FROM Jobs u WHERE u.orderdetails.orderDetailId = :orderDetailId AND u.status.status_id = 16")
    List<Jobs> getJobByOrderDetailIdEmployeeSick(int orderDetailId);
...
//    List<Jobs> filterJobWasDoneByEmployeeName(String keyword);
    ...
    @Query("SELECT new com.example.demo.Dto.OrderDTO.JobProductDTO(j.jobId,j.code,null, COALESCE(p.productId, 0) ,COALESCE(p.productName, '') ,  COALESCE(p.description, ''),COALESCE(j.cost, 0) , j.status,COALESCE(j.quantityProduct, 0) ," +
            "COALESCE(u.userId, 0), COALESCE(u.username, ''), COALESCE(pos.position_id, 0), COALESCE(pos.position_name, ''),COALESCE(MIN(e.processProductErrorId),0L),COALESCE(j.reassigned, false),o.contractDate,o.orderDate) " +
            "FROM Jobs j " +
